...
import com.foomei.common.number.RandomUtil;
...
public class CryptoUtil {
     ...
     * @param key 密钥
     */
    public static String hmacSha1(String input, String password) {
        byte[] result = hmacSha1(StringUtils.getBytesUtf8(input), StringUtils.getBytesUtf8(password));
     ...
     * @param key 密钥
     */
    public static boolean validHmacSha1(String expected, String input, String password) {
        String actual = hmacSha1(input, password);
    ...
    public static String aesEncrypt(String input, String password) {
        byte[] key = generateAesKey(password);
        byte[] result = aes(StringUtils.getBytesUtf8(input), key, Cipher.ENCRYPT_MODE);
        return EncodeUtil.encodeBase64UrlSafe(result);
    }
    ...
    public static String aesDecrypt(String input, String password) {
        byte[] key = generateAesKey(password);
        byte[] result = aes(EncodeUtil.decodeBase64(input), key, Cipher.DECRYPT_MODE);
	...
	/**
     * 根据密码生成AES密钥,返回字节数组, 默认长度为128位(16字节).
     */
    public static byte[] generateAesKey(String password) {
        return generateAesKey(DEFAULT_AES_KEYSIZE, password);
    }
    
    /**
     * 根据密码生成AES密钥,可选长度为128,192,256位.
     */
    public static byte[] generateAesKey(int keysize, String password) {
        try {
            KeyGenerator keyGenerator = KeyGenerator.getInstance(AES_ALG);
            keyGenerator.init(keysize, new SecureRandom(StringUtils.getBytesUtf8(password)));
            SecretKey secretKey = keyGenerator.generateKey();
            return secretKey.getEncoded();
        } catch (Exception e) {
            throw ExceptionUtil.unchecked(e);
