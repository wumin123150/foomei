...
public class ExceptionUtil {
	...
	/**
     * 拼装 短异常类名: 异常信息 location at 第一次出现包名的位置
     */
	public static String toStringWithLocation(@Nullable Throwable t, @Nullable String packageName) {
        if (t == null || StringUtils.isEmpty(packageName)) {
        ...
        if (StringUtils.isNotEmpty(packageName)) {
            for (int i = 1; i < stackFrames.length; i++) {
                stackFrame = StringUtils.trimToEmpty(stackFrames[i]);
                if(StringUtils.startsWith(stackFrame, "at " + packageName)) {
                    location = stackFrame;
                    break;
                }
            }
        } 
        sb.append("; location ").append(location == null ? StringUtils.trimToEmpty(stackFrames[1]) : location);
