...
public class SpeedSpringProcessor implements BeanFactoryPostProcessor {
  ...
  private boolean needRemove(String beanName, BeanDefinition beanDefinition) {
    ...
    //spring data jpa
    if(className.equals("org.springframework.data.jpa.repository.support.JpaRepositoryFactoryBean")) {
      PropertyValue repositoryInterfaceValue = beanDefinition.getPropertyValues().getPropertyValue("repositoryInterface");
